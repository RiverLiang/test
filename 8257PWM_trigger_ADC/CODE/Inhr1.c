...
void Inhr1_InterruptRx(void)
{
  ...
  Data = (Inhr1_TComData)getReg(QSCI1_DATA); /* Read data from the receiver */
  if(StatReg & (QSCI1_STAT_OR_MASK)) { /* Is HW overrun error detected? */
    setReg(QSCI1_STAT, 0);             /* Reset error request flags */
...
void Inhr1_InterruptError(void)
{
  ...
  if (StatReg & QSCI1_STAT_OR_MASK) {  /* Is overrun error flag set? */
    OnFlags |= OVERRUN_ERR;            /* If yes then set the flag */
  }
  if (StatReg & QSCI1_STAT_NF_MASK) {  /* Is noise error flag set? */
    OnFlags |= NOISE_ERR;              /* If yes then set the flag */
  }
  if (StatReg & QSCI1_STAT_FE_MASK) {  /* Is framing error flag set? */
    OnFlags |= FRAMING_ERR;            /* If yes then set the flag */
  }
  if (StatReg & QSCI1_STAT_PF_MASK) {  /* Is parity error flag set? */
